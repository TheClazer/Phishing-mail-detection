...
import { ShieldCheckIcon, ShieldExclamationIcon, InformationCircleIcon, LoaderIcon, BrainIcon, GoogleIcon } from './icons';
...
export const ResultDisplay: React.FC<ResultDisplayProps> = ({ result, isLoading, loadingMessage, onDetailedAnalysis, onSearchGrounding }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {(result.status === Status.SAFE || result.status === Status.PHISHING) && (
        <div className="flex flex-col sm:flex-row gap-4">
          <button
            onClick={onDetailedAnalysis}
            disabled={isLoading || !!result.detailedAnalysis}
          ...
          >
            {isLoading && loadingMessage.includes('in-depth') ? <LoaderIcon className="animate-spin mr-2 h-4 w-4" /> : <BrainIcon className="mr-2 h-4 w-4" />}
            In-depth Analysis
          </button>
          <button
            onClick={onSearchGrounding}
            disabled={isLoading || !!result.searchAnalysis}
            className="flex-1 inline-flex items-center justify-center px-4 py-2 border border-gray-300 dark:border-gray-500 text-sm font-medium rounded-md shadow-sm text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {isLoading && loadingMessage.includes('Google') ? <LoaderIcon className="animate-spin mr-2 h-4 w-4" /> : <GoogleIcon className="mr-2 h-4 w-4" />}
            Check with Google
          </button>
        </div>
      )}
      ...
      {result.searchAnalysis && (
         <ResultCard title="Google Search Grounding" icon={<GoogleIcon className="h-5 w-5 text-blue-500" />}>
           <p>{result.searchAnalysis}</p>
